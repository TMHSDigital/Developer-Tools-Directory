...
- **`publish.yml` replaces plugin `release.yml` for library servers**. Library MCP servers release via npm publish (OIDC provenance recommended; see [versioning.md](versioning.md)). A conventional-commit plugin `release.yml` that reads `plugin.json` does not apply. Library servers still emit an mcp `release.yml` that tags and then dispatches `publish.yml`.
- **Deployed-service archetype.** An mcp-server that is a deployed HTTP service (no npm artifact) declares `{"archetype": "deployed-service"}` in repo-local `.drift-check.json`. That declaration is the only way to drop `publish.yml` from the required set. The checker does not infer the archetype from a Dockerfile, `"private": true`, or an absent package `name`. Missing `.drift-check.json` is the library default (`publish.yml` still required). Unknown `archetype` values are errors. The exemption is `publish.yml` and the catalog `npm` field only; `drift-check.yml` and `stale.yml` stay required. See [mcp-server.md](mcp-server.md).
